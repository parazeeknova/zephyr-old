...
export function useFollowUserMutation() {
  ...
  return useMutation({
    ...
    onMutate: async (userId) => {
      ...
      const previousFollowedUsers = queryClient.getQueryData<UserData[]>(['followed-users']) || [];
      const previousSuggestedConnections =
        queryClient.getQueryData<UserData[]>(['suggested-connections']) || [];

      const userToFollow = previousSuggestedConnections.find((user) => user.id === userId);
      ...
      return { previousFollowedUsers, previousSuggestedConnections, userToFollow };
    },
    onError: (err, userId, context) => {
      queryClient.setQueryData(['followed-users'], context?.previousFollowedUsers);
      queryClient.setQueryData(['suggested-connections'], context?.previousSuggestedConnections);
      toast({
        title: 'Error',
    ...
    onSuccess: (followedUser, userId, context) => {
      const displayName = context?.userToFollow?.displayName || followedUser.displayName;
      toast({
        title: 'Success',
...
export function useUnfollowUserMutation() {
  ...
  return useMutation({
    ...
    onMutate: async (userId) => {
      ...
      const userToUnfollow = previousFollowedUsers.find((user) => user.id === userId);
      ...
      return { previousFollowedUsers, previousSuggestedConnections, userToUnfollow };
    },
    onError: (err, userId, context) => {
      queryClient.setQueryData(['followed-users'], context?.previousFollowedUsers);
      queryClient.setQueryData(['suggested-connections'], context?.previousSuggestedConnections);
    ...
    onSuccess: (unfollowedUser, userId, context) => {
      const displayName = context?.userToUnfollow?.displayName || unfollowedUser.displayName;
      toast({
        title: 'Success',
